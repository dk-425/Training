...
//------------------------Parameter----------------------
localparam
    TV_IN  = "../tv/cdatafile/c.cyclicPrefixRemoval.autotvin_input_r.dat", 
    TV_OUT = "../tv/rtldatafile/rtl.cyclicPrefixRemoval.autotvout_input_r.dat"; 
//------------------------Local signal-------------------
parameter DATA_WIDTH = 32'd 46;
parameter ADDR_WIDTH = 32'd 14;
parameter DEPTH = 32'd 8800;
parameter DLY = 0.1;
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[5] == 0) && (ce0 == 1) && (ce1 == 1) && (we1[5] == 1) && (address0 == address1))
	      dout0[5*8+5:5*8] <= #DLY din1[5*8+5:5*8];
	  else if (ce0 == 1)
	      dout0[5*8+5:5*8] <= #DLY mem[address0][5*8+5:5*8];
        else ;
    end
end
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[5] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[5] == 0) && (address0 == address1))
            dout1[5*8+5:5*8] <= #DLY din0[5*8+5:5*8];
	  else if (ce1 == 1)
            dout1[5*8+5:5*8] <= #DLY mem[address1][5*8+5:5*8];
        else ;
    end
end
...
always @ (posedge clk) begin
    if((we0[5] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[5] == 1) && (address0 == address1))
        mem[address0][5*8+5:5*8] <= #DLY din1[5*8+5:5*8];
    else if ((we0[5] == 1) && (ce0 == 1))
        mem[address0][5*8+5:5*8] <= #DLY din0[5*8+5:5*8];
end

always @ (posedge clk) begin
    if ((ce1 == 1) && (we1[5] == 1))
        mem[address1][5*8+5:5*8] <= #DLY din1[5*8+5:5*8];
end
