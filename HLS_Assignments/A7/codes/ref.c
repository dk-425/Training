...
int main( void )
{
    ...
    in_fid = fopen( "input.wav", "rb" );
    if ( in_fid == 0 ) {
        printf("couldn't open input.wav");
        return -1;
    }

    // open the output waveform file
    out_fid = fopen( "outref.wav", "wb" );
    if ( out_fid == 0 ) {
        printf("couldn't open outputFloat.wav");
        return -1;
    }
  ...
  // process all of the samples
    ...
    //    printf("%d\n",input);
        ...
        //printf("%d",output);
        int x=(int) output;
        block_fir(input, output0, coeffs);
       // printf("%d",output0);
        int y=(int) output0;
        if (abs((x-y)/x) <5){
        	p=p+1;
        	fprintf(outres,"PASS %p\n",(void*) *output0);
        }
        else{
        	flag=1;
        	fprintf(outres,"FAIL\n");}
    ...
    fclose( outres );
    if (flag==1){
    	printf("ERROR! ALL TEST CASES ARE NOT PASSED\n");
    }
    else{
