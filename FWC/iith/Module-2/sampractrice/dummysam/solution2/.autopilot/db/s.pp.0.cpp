...
__attribute__((sdx_kernel("crc24a", 0))) void crc24a(hls::stream<ap_axiu<8,0,0,0>>& input, hls::stream<typo>& output);
# 2 "s.cpp" 2

__attribute__((sdx_kernel("crc24a", 0))) void crc24a(hls::stream<ap_axiu<8,0,0,0>>& input, hls::stream<typo>& output) {
#line 15 "/home/sam-admin/Desktop/iith/Module-2/sampractrice/dummysam/solution2/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=crc24a
# 3 "s.cpp"

#line 6 "/home/sam-admin/Desktop/iith/Module-2/sampractrice/dummysam/solution2/directives.tcl"
#pragma HLSDIRECTIVE TOP name=crc24a
# 3 "s.cpp"
...
#pragma HLS UNROLL
 crc[i+u] = d.data(i,i);
   ...
   loop2222: for (int i=0;i<25 -1;i++){
#pragma HLS UNROLL
 crc[i+u] = 0;
     oput[i+u] = 0;
    }
   ap_uint<1> aee[p];
   VITIS_LOOP_34_1: for (int i=0;i<p;i++){
    aee[i]=crc[i];
   }
   ...
   loop3: for (int i = 0; i <= p - 25; i++) {
#pragma HLS PIPELINE
 if (crc[i] == 1 && d.last==1) {
          loop4: for (int j = 0; j < 25; j++) {
#pragma HLS UNROLL
 int k=i+j;
